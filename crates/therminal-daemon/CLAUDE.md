...
| `terminal.agents.get_session_detail` | Observer | Get the enriched Claude Code session detail for a pane (tn-ifee): session_title, current_tool, working_dir, context_percent, model, environment. Sourced from the per-pane `PaneCapacityCache` populated primarily by OSC 1341 markers (tn-nrur) and secondarily by the Claude state poller from `/tmp/claude-code-state/`. The `environment` field (tn-ncmj) carries the shell's cooperative self-identification (`wsl:<distro>`, `docker:<hostname>`, `ssh:<hostname>`, or `local`). Every field is nullable — absence is normal when hooks aren't installed or haven't ticked yet. Returns an error only if the pane does not exist. |
...
Claude integration lives in `crates/therminal-harness-claude/` — see its `CLAUDE.md` for the JSONL tailer, state watcher, event pipeline, and the `claude-events` dev binary. The daemon instantiates [`ClaudeHarness`](../therminal-harness-claude/src/lib.rs) at startup from `ensure.rs` and hands the resulting broadcast sender to the MCP server for the `therminal://claude/events` resource. The per-pane capacity cache in `src/pane_capacity.rs` is populated from two sources: (1) OSC 1341 markers via `upsert_from_marker()` (primary, pane-scoped, no PID resolution needed), and (2) `ClaudeStateUpdate`s from the file poller (fallback, suppressed when marker data is fresh). State→pane resolution uses a three-tier cascade: PID match → `session_id` lookup in the capacity cache → cwd fallback with trailing-slash normalization (tn-y2d8, tn-019y). Capacity entries are evicted immediately on state file removal (tn-q9kx) and swept by a 60s TTL on every poll tick.
