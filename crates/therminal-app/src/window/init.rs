...
impl App {
    pub(super) fn new(event_proxy: EventLoopProxy<UserEvent>) -> Self {
        ...
        let swarm_pane_session_ids: Option<crate::pane::swarm_watcher::PaneSessionIdProvider> =
            if config.general.auto_tile
                && config.general.swarm_watch_scope
                    == therminal_core::config::SwarmWatchScope::Current
            {
        ...
        let system_metrics = if config.widgets.system_metrics.enabled {
            ...
            let wsl_interval = std::time::Duration::from_millis(mc.wsl_poll_interval_ms.max(1000));
