//! MCP stdio bridge — connects stdin/stdout to the daemon's MCP Unix socket.
//!
...
///
/// Connects to the daemon's MCP Unix socket and copies bytes bidirectionally
/// between stdin/stdout and the socket. Exits when either side closes.
pub fn run() -> Result<()> {
...
async fn run_async() -> Result<()> {
    ...
    // Copy bidirectionally: stdin → socket and socket → stdout.
    // When either direction hits EOF or error, we shut down.
    tokio::select! {
