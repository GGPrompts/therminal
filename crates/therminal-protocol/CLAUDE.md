...
- **`PROTOCOL_VERSION`** -- Bumped when IPC wire format changes in a way that requires daemon restart. Normal rebuilds reuse the running daemon. Current bump note: `SplitPane` gained `startup_command`, so old daemons must be restarted before new clients use it.
