...
- **`DaemonEvent`** -- Server-pushed events: `StateChanged`, `SessionCreated`, `SessionDestroyed`, `PaneOutput`, `WorkspaceChanged`, `PaneExited`, `PaneResized`, `AgentChanged`, `SubagentStarted`, `SubagentStopped`. Clients filter via `EventKind`. `SubagentStarted`/`SubagentStopped` (tn-s8w3) carry `pane_id`, `agent_id`, and optionally `parent_session_id`/`jsonl_path` for hook-driven subagent auto-tile.
